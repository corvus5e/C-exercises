...
int day_of_year(int year, int month, int day){
    ...
    if(month < 0 || month > 13 || day < 1 || *(*daytab + sizeof(*daytab)*leap + month) < day)
        return -1;

    for(;--month > 0; day += *(*daytab + sizeof(*daytab)*leap + month))
        ;
...
void month_day(int year, int year_day, int *month, int *day){
    ...
    *month = 0;
    *day = year_day;
    while(*day > *(*daytab + sizeof(*daytab)*leap + (++(*month)))){
        *day -= *(*daytab + sizeof(*daytab)*leap + *month);
    }
}
